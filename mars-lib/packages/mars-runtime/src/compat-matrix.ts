...
export const bunApiCompatMatrix: BunApiCompatEntry[] = [
  ...
  },
  {
    ...
    status: "partial",
    phase: "M3",
    notes: "WebCrypto-backed sha1/sha256/sha512 digest path, Mars md5 fallback and digest encodings are covered; Bun's synchronous digest parity is pending.",
    tests: ["Phase 3 CryptoHasher digests common algorithms", "Phase 3 CryptoHasher supports digest encodings"],
  },
  {
    api: "Bun.password",
    status: "partial",
    phase: "M3",
    notes: "WebCrypto PBKDF2-SHA256 hash/verify fallback is covered, including explicit iterations and cost-to-iterations mapping; Bun bcrypt/argon2 compatibility is pending.",
  ...
  },
  {
    ...
    status: "partial",
    phase: "M3",
    notes: "randomUUID, randomBytes, async createHash digest, async createHmac digest and common digest encodings are covered; full Node synchronous Hash/Hmac parity is pending.",
