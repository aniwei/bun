...
- 总完成百分比: 79%
- 测试状态: Pass (`bun run check`: 118 pass / 0 fail / 689 expect calls)
...
| M3-03 | In Progress | 82% | Pass | `mars-lib/packages/mars-kernel/src/stdio.ts`, `mars-lib/packages/mars-kernel/src/kernel.ts`, `mars-lib/packages/mars-kernel/src/process-worker-factory.ts`, `mars-lib/packages/mars-runtime/src/process-worker-bootstrap.ts` | 实现 stdin/stdout/stderr stream 桥接、backpressure 和关闭语义。 | 已覆盖 ProcessHandle stdin readable、`write()` 输入、初始 string/ReadableStream `stdin` 输入、初始 stdin stream close、stdout/stderr readable、WritableStream mirror，补齐 Process Worker native carrier 的 `process.worker.stdin` / `process.worker.stdin.close` / stdout/stderr/exit 消息协议，让 configured Process Worker spawn 将 `ProcessHandle.write()` 与初始 stdin 转发到 worker，并在初始 stdin 结束时关闭 worker 内部 pid stdin；`bun run <entry>` 后 console stdout/stderr 会转发为 worker 输出消息，`kernel.spawn({ kind: "worker" })` 可自动把 Process Worker stdout/stderr/exit 回灌到 Kernel pid；完整 streaming backpressure 精细语义待补。 |
| M3-04 | In Progress | 72% | Pass | `mars-lib/packages/mars-runtime/src/install-global.ts`, `mars-lib/packages/mars-client/src/runtime.ts` | 实现 `Bun.spawn()` 到 Process Worker 或内置命令的映射。 | 当前 `Bun.spawn({ cmd: ["bun", "run", "index.ts"] })`、`runtime.spawn("bun", ["run", "index.ts"])` 与通用 shell 命令（如 `echo`）均可复用内存态 Kernel pid，命令结果会回灌到 ProcessHandle stdout/stderr/exit；新增 native-capable Process Worker factory 配置路径，`runtime.spawn("bun", ["run", "index.ts"])` 与 `Bun.spawn({ cmd: ["bun", "run", "index.ts"] })` 均可创建 Process Worker、转发 ProcessHandle stdin/初始 string stdin/初始 ReadableStream stdin、转发初始 stdin close，并回灌 worker stdout/stderr/exit；完整 stdin backpressure 细节待补。 |
| M3-05 | In Progress | 40% | Pass | `mars-lib/packages/mars-runtime/src/install-global.ts`, `mars-lib/packages/mars-runtime/src/types.ts` | 实现 `Bun.spawnSync()` 的 SAB 路径和无 SAB 降级错误。 | 已补 capability-aware fallback：无 SAB/Atomics.wait 时返回明确 requirement 错误；SAB 可用但未接 sync executor 时返回明确 not-wired 错误，并预留 `RuntimeContext.spawnSync` 扩展点；SAB 真同步执行路径待补。 |
...
| M3-09 | In Progress | 76% | Pass | `mars-lib/packages/mars-crypto/src/hasher.ts`, `mars-lib/packages/mars-runtime/src/install-global.ts` | 实现 `Bun.CryptoHasher` 常用算法，优先 sha1/sha256/sha512/md5。 | 已通过 WebCrypto 覆盖 sha1/sha256/sha512 async digest，并为 md5 补 Mars fallback，覆盖 hex/base64/base64url/buffer 输出编码，接入 `Bun.CryptoHasher`、`createHashDigest`、`node:crypto.createHash` 和 playground fixture；同步 digest 兼容待补。 |
| M3-10 | In Progress | 58% | Pass | `mars-lib/packages/mars-crypto/src/password.ts`, `mars-lib/packages/mars-runtime/src/install-global.ts` | 实现 `Bun.password.hash()`、`verify()` 的浏览器兼容路径。 | 已通过 WebCrypto PBKDF2-SHA256 fallback 覆盖 hash/verify、显式 iterations 和 cost-to-iterations 映射；Bun bcrypt/argon2 格式兼容与精确 cost 语义待补。 |
| M3-11 | In Progress | 66% | Pass | `mars-lib/packages/mars-node/src/crypto.ts` | 实现 `node:crypto` 验收路径子集，如 createHash、createHmac、randomUUID、randomBytes。 | 已覆盖 randomUUID、randomBytes、sha/md5 async createHash digest、async createHmac digest 与常见 digest encoding；完整同步 Hash/Hmac、cipher/sign/verify 待补。 |
