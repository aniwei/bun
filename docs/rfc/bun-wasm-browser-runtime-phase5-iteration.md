...
**当前测试**：历史基线 571+ 通过（27 个测试文件）；本轮已执行 `packages/bun-browser/test/integration.test.ts`，95/95 通过（含 semver + integrity + glob 新增回归用例）；补充回归 `bun test ./packages/bun-browser/test/node-http-shim.test.ts ./packages/bun-browser/test/serve-static.test.ts`，**41/41 通过**；`/tmp/t5174-vfs-check.ts` 运行通过。
**依赖文档**：
...
| 日期 | 作者 | 变更 |
|------|------|------|
| 2026-04-23 | claude | **当前进度同步（文档更新）**：补充最新验证结果 —— `node-http-shim.test.ts` + `serve-static.test.ts` 共 **41/41 通过**；`/tmp/t5174-vfs-check.ts` 运行通过；分支 `claude/bun-browser-wasm-runtime-fixes` 已推送。后续优先级维持：T5.16.4（JSX 入口兼容验证）、T5.18.4/5（braces/dependency 真身替换）、T5.15.1（threads wasm CI lane）。 |
