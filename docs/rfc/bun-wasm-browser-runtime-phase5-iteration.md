...
**立刻起步推荐**：T5.10.3（`bun_brace_expand`）和 T5.10.5（VLQ 真身）均已完成，T5.10.6（文档修正）亦同步完成。**Phase 5.13 T5.13.1–T5.13.5 已全部完成**（`bun_shell_parse` Zig 解析器 + `ShellInterpreter` TS 执行引擎 + `createShell`/`$` 模板标签）。当前绿线 **523/523**（25 files）。下一步建议：T5.10.4（`dependency.zig` 剥离 JSC 方法，工作量小，可行性高）；T5.14（预览闭环，依赖 T5.11.1/5 均已完成）；T5.12（阻塞 I/O，依赖 wasm-threads 构建产物）。
