...
import { CommandRegistry, defaultRegistry } from '../src/shell-command-registry'
...
/**
 * Build a minimal mock Kernel whose file I/O methods mirror the real Kernel
 * (direct methods, not under a `.fs` sub-object).
 */
function makeMockKernel(fsFiles: Record<string, string> = {}) {
  ...
  return {
    ...
    async readFile(path: string, _enc?: string): Promise<string> {
      if (!(path in vfs)) throw Object.assign(new Error(`ENOENT: ${path}`), { code: 'ENOENT' })
      return vfs[path]!
    },
    ...
    async readdir(path: string): Promise<{ name: string; type: 'file' | 'directory' }[]> {
      ...
      for (const key of Object.keys(vfs)) {
        if (key.startsWith(prefix)) {
          ...
          entries.set(first, rel.includes('/') ? 'directory' : 'file')
        }
      }
    ...
    async rm(path: string, _opts?: any): Promise<void> {
      for (const key of Object.keys(vfs)) {
        if (key === path || key.startsWith(path + '/')) delete vfs[key]
      }
    },
    async rename(src: string, dst: string): Promise<void> {
      if (!(src in vfs)) throw Object.assign(new Error(`ENOENT: ${src}`), { code: 'ENOENT' })
      vfs[dst] = vfs[src]!
      delete vfs[src]
    },
...
// ── 2b. CommandRegistry（T5.13.3 扩展）──────────────────────────────────────

describe('CommandRegistry', () => {
  test('defaultRegistry 包含所有内置命令', () => {
    const names = [...defaultRegistry.list().keys()]
    for (const name of ['echo', 'cd', 'pwd', 'ls', 'cat', 'mkdir', 'rm', 'cp', 'mv',
                        'env', 'export', 'true', 'false', ':', 'printf', 'help']) {
      expect(names).toContain(name)
    }
  })

  test('register 注册新命令并可 resolve', () => {
    const reg = new CommandRegistry()
    const fn = () => ({ exitCode: 0, stdout: 'hi\n', stderr: '' })
    reg.register('greet', fn, { description: 'say hi' })
    expect(reg.resolve('greet')).toBe(fn)
    expect(reg.list().get('greet')?.meta.description).toBe('say hi')
  })

  test('register 覆盖已有命令', () => {
    const reg = new CommandRegistry()
    const fn1 = () => ({ exitCode: 0, stdout: 'v1\n', stderr: '' })
    const fn2 = () => ({ exitCode: 0, stdout: 'v2\n', stderr: '' })
    reg.register('cmd', fn1)
    reg.register('cmd', fn2)
    expect(reg.resolve('cmd')).toBe(fn2)
  })

  test('unregister 移除命令', () => {
    const reg = new CommandRegistry()
    reg.register('tmp', () => ({ exitCode: 0, stdout: '', stderr: '' }))
    expect(reg.resolve('tmp')).toBeDefined()
    expect(reg.unregister('tmp')).toBe(true)
    expect(reg.resolve('tmp')).toBeUndefined()
    expect(reg.unregister('tmp')).toBe(false)
  })

  test('resolve 未知命令返回 undefined', () => {
    const reg = new CommandRegistry()
    expect(reg.resolve('notacommand')).toBeUndefined()
  })

  test('ShellInterpreter 使用自定义注册表执行注入命令', async () => {
    if (!hasShellParse) return
    const kernel = makeMockKernel()
    const reg = new CommandRegistry()
    reg.register('greet', (args) => ({
      exitCode: 0,
      stdout: `hello ${args[0] ?? 'stranger'}\n`,
      stderr: '',
    }))
    const interp = new ShellInterpreter(kernel, reg)
    const ast = rt.shellParse('greet world')!
    const result = await interp.run(ast)
    expect(result.exitCode).toBe(0)
    expect(result.stdout).toBe('hello world\n')
  })

  test('沙箱注册表：移除危险命令后 rm 变为未知命令', async () => {
    if (!hasShellParse) return
    const kernel = makeMockKernel({ '/important.txt': 'keep me' })
    // Build sandbox registry from defaultRegistry, excluding rm
    const sandbox = new CommandRegistry()
    for (const [name, entry] of defaultRegistry.list()) {
      if (name !== 'rm') sandbox.register(name, entry.fn, entry.meta)
    }
    const interp = new ShellInterpreter(kernel, sandbox)
    const ast = rt.shellParse('rm /important.txt')!
    const result = await interp.run(ast)
    // Falls through to kernel.process() which resolves 0 in mock, but file is untouched
    expect(kernel._vfs['/important.txt']).toBe('keep me')
  })
})
