...
describe('bun_hash', () => {
  ...
  test("SHA-1 of 'hello'", async () => {
    ...
    expect(Buffer.from(digest).toString('hex')).toBe('aaf4c61ddcc5e8a2dabede0f3b482cd9aea9434d')
  })
...
describe('bun_inflate', () => {
  test('gzip inflate roundtrip', async () => {
    const rt = await makeRuntime()
    const original = new TextEncoder().encode('Hello, WASM inflate! '.repeat(50))
