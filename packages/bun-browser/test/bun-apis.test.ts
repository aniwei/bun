...
async function makeRuntime(
  onPrint?: (data: string, kind: "stdout" | "stderr") => void,
): Promise<WasmRuntime> {
  ...
  const transpile = (src: string, filename: string): string => {
    try { return tsTranspiler.transformSync(src); } catch { return src; }
  };
...
function evalCode(rt: WasmRuntime, code: string): number {
  ...
  rt.withString(code, (sp, sl) => {
    rt.withString("<test>", (fp, fl) => { rc = evalFn(sp, sl, fp, fl); });
  });
  return rc;
}
...
describe("Bun.file", () => {
  ...
  test("arrayBuffer() 返回 ArrayBuffer", async () => {
    ...
    evalCode(rt, `Bun.file("/ab.txt").arrayBuffer().then(function(ab){ console.log("AB=" + (ab instanceof ArrayBuffer) + ":" + ab.byteLength); });`);
