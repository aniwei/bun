...
function makeFakeFetch(registry: Record<string, FakeRegistryEntry>): typeof globalThis.fetch {
...
describe('installPackages (端到端)', () => {
  test('装一个最简包：metadata + tarball + 解压到 /node_modules/', async () => {
    ...
    expect(paths).toEqual(['/node_modules/left-pad/index.js', '/node_modules/left-pad/package.json'])
    const indexFile = result.files.find(f => f.path.endsWith('index.js'))!
    const data = typeof indexFile.data === 'string' ? indexFile.data : new TextDecoder().decode(indexFile.data)
  ...
  test('多包：按精确版本与 caret 混合解析', async () => {
    ...
    expect(result.packages.map(p => `${p.name}@${p.version}`).sort()).toEqual(['bar@2.3.4', 'foo@1.0.0'])
  ...
  test('传递依赖：BFS 展开 dependencies，WASM 决策路径', async () => {
    ...
    const result = await installPackages({ app: '1.0.0' }, { fetch: fetchFn, registry: 'https://fake-registry' })
...
describe('bun_npm_parse_metadata (T5.4.1)', () => {
  ...
  function makeNpmMeta(
    ...
    return JSON.stringify({
      ...
      versions: Object.fromEntries(Object.entries(versions).map(([v, meta]) => [v, { version: v, ...meta }])),
    })
  }
...
describe('bun_npm_resolve_graph (T5.4.2)', () => {
  test('单包解析', async () => {
    ...
    const result = rt.resolveGraph({ react: '^18.0.0' }, { react: makeNpmMeta('react', '18.2.0') })
    expect(result).not.toBeNull()
...
describe('bun_html_rewrite (T5.7.3)', () => {
  test('替换 script[src] 属性', async () => {
    ...
    const result = rt.htmlRewrite(html, [{ selector: 'script[src]', attr: 'src', replace: 'new.js' }])
    expect(result).not.toBeNull()
  ...
  test('无匹配规则：HTML 原样返回', async () => {
    ...
    const result = rt.htmlRewrite(html, [{ selector: 'script[src]', attr: 'src', replace: 'x.js' }])
    expect(result).not.toBeNull()
  ...
  test('set_text 替换标签文本内容', async () => {
    ...
    const result = rt.htmlRewrite(html, [{ selector: 'title', text: 'New Title' }])
    expect(result).not.toBeNull()
