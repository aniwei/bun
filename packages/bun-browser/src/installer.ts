/**
 * Browser-side npm installer.
 *
 * 走宿主 fetch() 直连 npm registry（CORS 已开启），解压用 DecompressionStream("gzip") + ustar parser。
 * `installPackages()` 返回 VfsFile[]，可直接由 `buildSnapshot()` 写入 WASM VFS。
 */
...
/**
 * 解析 dependencies 表，BFS 展开传递依赖。
 * `resolveTransitive` 缺省 true；设为 false 时仅安装顶层 deps。
 */
...
class Installer {
  ...
  async installOne(
    ...
    // 若 extractTgz 可用，直接在 WASM VFS 内解压，无需 snapshot 往返。
...
// ──────────────────────────────────────────────────────────
//
// 支持：完全相等、dist-tag、caret、tilde、通配符、>=。
// 不支持：区间（A || B）、`<` `<=` `>`、预发布版本顺序细节。
//
...
function pickHighestStable(versions: string[]): string | null {
  const stable = versions.map(v => ({ v, p: parseSemver(v) })).filter(x => x.p !== null && x.p!.pre.length === 0)
...
function rangeMatch(op: '^' | '~' | '>=' | string, base: ParsedSemver, v: ParsedSemver): boolean {
  ...
  if (op === '>=') return true
  if (op === '^') return v.major === base.major && v.major !== 0 ? true : v.major === 0 && v.minor === base.minor
