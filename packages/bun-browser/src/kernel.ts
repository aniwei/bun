...
// ---------------------------------------------------------------------------
// T5.11.2：ProcessHandle —— WebContainer 兼容的进程句柄
// ---------------------------------------------------------------------------
...
export class ProcessHandle {
  ...
  /**
   * 终止进程。
   * 当前为 stub（T5.12.3 实现真实 SIGTERM/SIGKILL via SAB signal slot）。
   */
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  kill(_signal?: string): void {
    // TODO: T5.12.3 — 通过 SAB header signal slot 发送信号给子 Worker
  }
...
// ---------------------------------------------------------------------------
// T5.11.1：Kernel 事件类型（on/off API）
// ---------------------------------------------------------------------------
...
export class Kernel {
  ...
  /** T5.11.2：进程句柄映射（streamOutput=true 的 spawn）。 */
  private pendingProcesses = new Map<string, ProcessHandle>()
  /** T5.11.1：port / server-ready 事件 listeners。 */
  ...
  constructor(private readonly opts: KernelOptions) {
    ...
    if (opts.threadsWasmModule) {
      ...
      if (cap.threadsReady) {
        threadsWasmModule = opts.threadsWasmModule
  ...
  // ---------------------------------------------------------------------------
  // T5.11.4：WebContainer 兼容的 FileSystemTree API
  // ---------------------------------------------------------------------------
