...
export async function createWasmRuntime(
  ...
  const rt: WasmRuntime = {
    ...
    globMatch(glob: string, path: string): boolean | null {
      const exports_ = _instance!.exports as Record<string, unknown>
      const matchFn = exports_.bun_glob_match as
        | ((gp: number, gl: number, pp: number, pl: number) => number)
        | undefined
      if (!matchFn) return null
      const alloc = exports_.bun_malloc as (n: number) => number
      const free_ = exports_.bun_free as (ptr: number) => void
      const mem = () => (_instance!.exports.memory as WebAssembly.Memory).buffer
      const gbytes = enc.encode(glob)
      const pbytes = enc.encode(path)
      const gPtr = alloc(Math.max(1, gbytes.byteLength))
      const pPtr = alloc(Math.max(1, pbytes.byteLength))
      if (gPtr === 0 || pPtr === 0) return null
      if (gbytes.byteLength > 0) new Uint8Array(mem(), gPtr, gbytes.byteLength).set(gbytes)
      if (pbytes.byteLength > 0) new Uint8Array(mem(), pPtr, pbytes.byteLength).set(pbytes)
      let code: number
      try {
        code = matchFn(gPtr, gbytes.byteLength, pPtr, pbytes.byteLength)
      } finally {
        free_(gPtr)
        free_(pPtr)
      }
