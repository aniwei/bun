...
/**
 * T5.12.3: UI → Kernel：向正在运行的 spawn 发送信号（in-process 或 ProcessManager 模式均适用）。
 *
 * - **In-process spawn**：kernel-worker 在收到此消息时通过设置 WASM 侧 `g_kill_signal` 标志
 *   请求提前退出（仅在下次 Zig 代码检查该标志时生效）。
 * - **ProcessManager spawn**：委托给 `ProcessManager.kill(id, signal)` 立即 terminate Worker。
 */
export interface SpawnKillRequest {
  kind: 'spawn:kill'
  /** 对应 SpawnRequest.id。 */
  id: string
  /** POSIX 信号编号（默认 15=SIGTERM）。9=SIGKILL, 2=SIGINT 亦合法。 */
...
// ---------------------------------------------------------------------------
// T5.12.5：VFS fs.watch 事件总线
// ---------------------------------------------------------------------------

/** UI → Kernel：订阅 VFS 路径的变更事件。 */
export interface FsWatchRequest {
  kind: 'fs:watch'
  /** 客户端分配的 watchId，用于路由后续事件与取消订阅。 */
  id: string
  path: string
  recursive?: boolean | undefined
}

/** UI → Kernel：取消 VFS 路径订阅。 */
export interface FsUnwatchRequest {
  kind: 'fs:unwatch'
  /** 对应 FsWatchRequest.id。 */
  id: string
}

/** Kernel → UI：VFS 文件/目录变更事件。 */
export interface FsWatchEvent {
  kind: 'fs:watch:event'
  /** 对应 FsWatchRequest.id。 */
  id: string
  eventType: 'rename' | 'change'
  /** 变更文件的绝对路径（VFS 路径，以 `/` 开头）。 */
  filename: string
}
...
/**
 * Kernel → UI：Bun.serve 服务器已停止（`server.stop()` 调用）。
 *
 * T5.14.1 — port close lifecycle event。
 * kernel-worker.ts 通过 Proxy 拦截 `delete globalThis.__bun_routes[port]` 并 postMessage 本事件。
 */
