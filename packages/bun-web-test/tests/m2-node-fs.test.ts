...
import { createRequire } from '../../../packages/bun-web-node/src/module'
...
describe('bun-web M2 node fs bridge smoke', () => {
  ...
  test('official replay: Stats supports call without new and keeps prototype methods', () => {
    const vfs = new VFS()
    const fs = createNodeFs(vfs)
    fs.mkdirSync('/stats', { recursive: true })
    fs.writeFileSync('/stats/a.txt', 'a')

    const stat = fs.statSync('/stats/a.txt')
    expect(stat.isFile()).toBe(true)
    expect(stat.isDirectory()).toBe(false)
    expect(typeof stat.mtimeMs).toBe('number')
    expect(stat.atime instanceof Date).toBe(true)
    expect(Object.getPrototypeOf(stat).constructor.name).toBe('Stats')
  })

  test('official replay: bun internal createStatsForIno is available', () => {
    const req = createRequire('/entry.js')
    const internal = req('bun:internal-for-testing') as {
      createStatsForIno: (ino: bigint, big: boolean) => { ino: number | bigint }
    }

    const numeric = internal.createStatsForIno(123n, false)
    const bigint = internal.createStatsForIno(123n, true)

    expect(numeric.ino).toBe(123)
    expect(bigint.ino).toBe(123n)
  })

  test('official replay: require("node:fs") exposes Stats constructor', () => {
    const req = createRequire('/entry.js')
    const nodeFs = req('node:fs') as {
      Stats: new (...args: number[]) => { ino: number }
      statSync: (path: string) => { isFile(): boolean }
    }

    const created = new nodeFs.Stats(0, 0, 0, 0, 0, 0, 0, 7, 0, 0, 0, 0, 0, 0)
    expect(created.ino).toBe(7)

    const vfs = new VFS()
    const fs = createNodeFs(vfs)
    fs.mkdirSync('/x', { recursive: true })
    fs.writeFileSync('/x/a.txt', 'a')
    expect(fs.statSync('/x/a.txt').isFile()).toBe(true)
  })

  test('official replay: promises read/write honor already-aborted signals', async () => {
    const vfs = new VFS()
    const fs = createNodeFs(vfs)
    const signal = AbortSignal.abort()

    await expect(fs.promises.readFile('/missing', { signal })).rejects.toMatchObject({
      name: 'AbortError',
      code: 'ABORT_ERR',
    })

    await expect(fs.promises.writeFile('/x.txt', 'x', { signal })).rejects.toMatchObject({
      name: 'AbortError',
      code: 'ABORT_ERR',
    })
  })
